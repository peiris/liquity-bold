...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async (accounts) => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      const carolTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const txA = await stabilityPool.withdrawETHGainToTrove(aliceTroveId, { from: alice });
      const txB = await stabilityPool.withdrawETHGainToTrove(bobTroveId, { from: bob });
      const txC = await stabilityPool.withdrawETHGainToTrove(carolTroveId, { from: carol });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "6666666666666666666666"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "6666666666666666666666"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "6666666666666666666666"),
        10000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "3333333333333333333333"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "3333333333333333333333"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "3333333333333333333333"),
        10000,
      );
    ...
    it("withdrawETHGainToTrove():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "0"), 10000);
    ...
    it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing Bold", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "6000000000000000000000"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "6000000000000000000000"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "6000000000000000000000"),
        10000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing Bold", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "4000000000000000000000"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "4000000000000000000000"),
        10000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "4000000000000000000000"),
        10000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "13333333333333333333333"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "20000000000000000000000"),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "5000000000000000000000"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "10000000000000000000000"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "15000000000000000000000"),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      */
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "901719380174061000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "205592018679686000000000"),
        10000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "5906261940140100000000"),
        10000000000,
      );
    ...
    it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 Bold.  A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      const dennisTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const txB = await stabilityPool.withdrawETHGainToTrove(bobTroveId, { from: bob });
      const txC = await stabilityPool.withdrawETHGainToTrove(carolTroveId, { from: carol });
      const txD = await stabilityPool.withdrawETHGainToTrove(dennisTroveId, { from: dennis });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "1666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "1666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "1666666666666666666666"),
        100000,
      );

      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "5000000000000000000000"),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 Bold.  A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "0"), 100000);
    ...
    it("withdrawETHGainToTrove(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      */
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "17832817337461300000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "5944272445820430000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "4458204334365320000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "11764705882352900000000"),
        100000000000,
      );
    ...
    it("withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 Bold.  A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "0"), 1000);
    ...
    it("withdrawETHGainToTrove(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      */
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "1672240802675590000000"),
        10000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "2090301003344480000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "1045150501672240000000"),
        100000000000,
      );
    ...
    it("withdrawETHGainToTrove(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 Bold. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct Bold deposit and ETH Gain", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(alice)).toString(), "666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "2000000000000000000000"),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "5000000000000000000000"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "5000000000000000000000"),
    ...
    it("withdrawETHGainToTrove(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const erinTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const txC = await stabilityPool.withdrawETHGainToTrove(carolTroveId, { from: carol });
      const txD = await stabilityPool.withdrawETHGainToTrove(dennisTroveId, { from: dennis });
      const txE = await stabilityPool.withdrawETHGainToTrove(erinTroveId, { from: erin });
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "8333333333333333333333"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "16666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(erin)).toString(), "25000000000000000000000"),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      const flynTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const harrietTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const grahamTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const txD = await stabilityPool.withdrawETHGainToTrove(dennisTroveId, { from: dennis });
      const txE = await stabilityPool.withdrawETHGainToTrove(erinTroveId, { from: erin });
      const txF = await stabilityPool.withdrawETHGainToTrove(flynTroveId, { from: flyn });
      const txG = await stabilityPool.withdrawETHGainToTrove(grahamTroveId, { from: graham });
      const txH = await stabilityPool.withdrawETHGainToTrove(harrietTroveId, { from: harriet });
      ...
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(erin)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(flyn)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await stabilityPool.getCompoundedBoldDeposit(graham)).toString(), "0"), 100000);
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(harriet)).toString(), "0"),
        100000,
      );
    ...
    it.skip("withdrawETHGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
      th.logBN("P", await stabilityPool.P()); // 8999999999, i.e. 1 wei less than expected
      ...
      // Expect Bob to retain 1% of initial deposit (100 Bold) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), "100000000000000000000"),
        100000,
      );
    ...
    it.skip("withdrawETHGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 liquidated.  Value of P reduced to 9e9
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
      th.logBN("P", await stabilityPool.P()); // P = 8999999999, i.e. 1 wei less than expected
      assert.equal((await stabilityPool.P()).toString(), dec(9, 9));
      ...
      */
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), dec(100, 18)),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), dec(200, 18)),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), dec(300, 18)),
        100000,
      );
    ...
    it.skip("withdrawETHGainToTrove(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(bob)).toString(), dec(1, 17)),
        100000,
      );
    ...
    it.skip("withdrawETHGainToTrove(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(carol)).toString(), dec(2, 17)),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), dec(3, 17)),
        100000,
      );
    ...
    */
    ...
    it.skip("withdrawETHGainToTrove(): Several deposits of 10000 Bold span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 liquidated. P decreases to 1e13
      const txL1 = await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
      assert.isTrue(txL1.receipt.status);
      th.logBN("P", await stabilityPool.P()); // P = 0.000009999999999999, 1 wei less than expected
      ...
      assert.equal(await stabilityPool.getCompoundedBoldDeposit(alice), "0");
      assert.equal(await stabilityPool.getCompoundedBoldDeposit(alice), "0");
      assert.equal(await stabilityPool.getCompoundedBoldDeposit(alice), "0");
      // D should retain around 0.9999 Bold, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await stabilityPool.getCompoundedBoldDeposit(dennis)).toString(), dec(99999, 12)),
        100000,
      );
    ...
    it("withdrawETHGainToTrove(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      const DTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const ETroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      const FTroveId = await th.openTroveWrapper(
        contracts,
        th._100pct,
        dec(10000, 18),
        ZERO_ADDRESS,
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txA = await stabilityPool.withdrawETHGainToTrove(ATroveId, { from: A });
      const txB = await stabilityPool.withdrawETHGainToTrove(BTroveId, { from: B });
      await priceFeed.setPrice(dec(100, 18));
      ...
      // Increasing the price for a moment to avoid pending liquidations to block withdrawal
      await priceFeed.setPrice(dec(200, 18));
      const txC = await stabilityPool.withdrawETHGainToTrove(CTroveId, { from: C });
      const txD = await stabilityPool.withdrawETHGainToTrove(DTroveId, { from: D });
      await priceFeed.setPrice(dec(100, 18));
      ...
      // Attempt withdrawals
      const txE = await stabilityPool.withdrawETHGainToTrove(ETroveId, { from: E });
      const txF = await stabilityPool.withdrawETHGainToTrove(FTroveId, { from: F });
    ...
    it("withdrawETHGainToTrove(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      const depositors = [alice, bob];
      for (const account of depositors) {
        await th.openTroveWrapper(contracts, th._100pct, dec(1, 36), account, account, 0, {
          from: account,
          value: dec(2, 27),
        });
      ...
      const txA = await stabilityPool.withdrawETHGainToTrove(th.addressToTroveId(alice), { from: alice });
      const txB = await stabilityPool.withdrawETHGainToTrove(th.addressToTroveId(bob), { from: bob });
      ...
      // Check Bold balances
      const aliceBoldBalance = await stabilityPool.getCompoundedBoldDeposit(alice);
      ...
      const bobBoldBalance = await stabilityPool.getCompoundedBoldDeposit(bob);
    ...
    it.skip("withdrawETHGainToTrove(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      const depositors = [alice, bob];
      for (const account of depositors) {
        await th.openTroveWrapper(contracts, th._100pct, dec(1, 38), account, account, {
          from: account,
          value: dec(2, 29),
        });
      ...
      const txAPromise = stabilityPool.withdrawETHGainToTrove(th.addressToTroveId(alice), { from: alice });
      const txBPromise = stabilityPool.withdrawETHGainToTrove(th.addressToTroveId(bob), { from: bob });
      ...
      th.logBN("aliceBoldBalDiff", aliceBoldBalDiff); // 100x bigger than expected
