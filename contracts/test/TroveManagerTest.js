...
contract("TroveManager", async (accounts) => {
  ...
  const getICR150 = (withBatchDelegation) => withBatchDelegation ? toBN(dec(150, 16)).add(toBN(dec(1,12))) : toBN(dec(150, 16));
  ...
  const testCorpus = (withBatchDelegation = false) => {
    ...
    describe(`${withBatchDelegation ? "With" : "Without"} delegation`, async () => {
      it("liquidate(): closes a Trove that has ICR < MCR", async () => {
        ...
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(4, 18)), extraParams: { from: alice, annualInterestRate: dec(5, 16), batchManager: getBatchManager(withBatchDelegation, dennis) } });
      ...
      it("liquidate(): increases DefaultPool ETH and Bold debt by correct amounts", async () => {
        ...
        th.assertIsApproximatelyEqual(defaultPool_ETH_After, defaultPool_ETH);
      ...
      it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
        ...
        const redemptionAmount = aliceEntireDebt.add(bobEntireDebt).add(carolEntireDebt).sub(toBN(dec(600, 18)));
      ...
      it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
        ...
        let bobEntireDebt = await troveManager.getTroveEntireDebt(bobTroveId);
        let carolEntireDebt = await troveManager.getTroveEntireDebt(carolTroveId);
        const redemptionAmount = bobEntireDebt.add(carolEntireDebt).sub(toBN(dec(400, 18)));
      ...
      it("checkBelowCriticalThreshold(): Returns true when TCR < 150%", async () => {
        ...
        await openTrove({ ICR: getICR150(withBatchDelegation), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        await openTrove({ ICR: getICR150(withBatchDelegation), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
      ...
      it("getTroveStake(): Returns stake", async () => {
        const { troveId: ATroveId, collateral: A_coll } = await openTrove({
          ICR: getICR150(withBatchDelegation),
          extraParams: { from: A, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: BTroveId, collateral: B_coll } = await openTrove({
          ICR: getICR150(withBatchDelegation),
